<template>
  ...
  <div class="main">
    <div class="title-section">
      ...
      <div>
        <batch-button :disabled="!checkedItems.length" v-if="user.hasPermission('delete')" :api="onBatchDelete" label="申请授权"></batch-button>
        <batch-button :disabled="!checkedItems.length" v-if="user.hasPermission('delete')" :api="onBatchDelete"></batch-button>
        <a-button type="primary" v-if="user.hasPermission('add')" @click="router.push('/my-logicsceneset/edit/0')">创建场景集</a-button>
      </div>
    </div>
    <div>
