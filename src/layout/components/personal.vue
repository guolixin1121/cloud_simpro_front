<template>
    <a-spin :spinning="loading">
        <div class="personal">
            <div>
                <span class="label">用户名</span><span class="w-4/5">{{ userDetail.username }}</span>
            </div>
            <div>
                ...
                <a-input ref="inputRef" v-else :maxlength="30" v-model:value="userDetail.name" @blur="onEditName"/>
            </div>
            <div>
                <span class="label">角色名称</span><span class="w-4/5">{{ userDetail.roleName }}</span>
            </div>
            <div>
                <span class="label">企业账户名</span>
                <span class=" w-4/5">{{ user.enterpriseAccount }}</span>
            </div>
            <div>
